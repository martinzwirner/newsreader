...
class importer {
  ...
  _processUrl(index) {
    ...
    logger.info('_processUrl: ' + (index + 1) + ' / ' + this._bookmarks.length, this._loggingContext);
    ...
    this._bookmarksTable.findOne({where: {url: url}})
  ...
  _createBookmark(importedBookmark, bookmark) {
    ...
    if (bookmark !== null) {
      ...
      if (importedBookmark.isViewed && !bookmark.isViewed) {
        ...
        logger.debug('updating isViewed for: ' + url, this._loggingContext);
