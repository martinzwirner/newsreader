...
const tweetRegex = /https:\/\/twitter.com\/.+\/status\/.+/;
...
module.exports = class requestManager {
  ...
  request(url) {
    ...
    if (tweetRegex.test(url)) {
      ...
      logger.debug("request: no tweet url", this._loggingContext);
      return this._requestViaHttp(url)
        .then(response => {
          if (tweetRegex.test(response.urlAfterRedirect)) {
            return this._requestTweet(response.urlAfterRedirect);
          } else {
            logger.debug("request: urlAfterRedirect is no tweet url", this._loggingContext);
            return response;
          }
        });
    }
  ...
  _requestViaHttp(url) {
    ...
    return request.get({
      ...
      followRedirect: (response) => {
        ...
        result.urlAfterRedirect = response.headers.location;
        logger.debug('_requestViaHttp: redirect to ', result.urlAfterRedirect, this._loggingContext);
        return true;
      }
    })
      .then((response) => {
      ...
      .catch((err) => {
        ...
        if (err.statusCode === 404) {
          ...
          logger.error('_requestViaHttp: error response from ' + url + ': ' + err, this._loggingContext);
        }
      })
      .then(() => result);
  }
