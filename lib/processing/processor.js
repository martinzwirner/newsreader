...
class processor {
  ...
  _process(bookmark) {
    ...
    .catch((response) => {
      newValues.errorStatusCode = response.code;
      if (response.code === 404) {
        logger.info('_process: page not found', this._loggingContext);
      } else {
        logger.error('_process: error response from ' + bookmark.url + ': ' + response.code + '\n' + response.body, this._loggingContext);
      }
    })
    .then((response) => {
      ...
      dom = new JSDOM(response.body, { virtualConsole: this._virtualConsole });
  ...
  _saveValues(bookmark, newValues) {
    ...
    const setLanguagePromise = this._database.getTable('language').findOne({ where: { code: languageCode }})
    .then((language) => {
    ...
    const setHostPromise = this._hostManager.get(hostname)
    .then((host) => {

      //logger.warn('_saveValues: got host id ' + host.id + ' for hostname ' + hostname, this._loggingContext);
